...
model_names.append('vgg16_bn_FConv')
model_names.append('vgg16_FConv')
model_names.append('resnet18_FConv')
model_names.append('resnet34_FConv')
model_names.append('resnet50_FConv')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.submission:
        ...
        ImageFolderWithIDs(testdir, transforms.Compose([
            transforms.Resize(256),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
            normalize,
...
def save_checkpoint(state, is_best,args, filename='_checkpoint.pth.tar'):
    torch.save(state, "results"+'/'+args.arch+filename)
    if is_best:
        shutil.copyfile('results'+'/'+args.arch+filename, 'results'+'/'+args.arch+'_model_best.pth.tar')
